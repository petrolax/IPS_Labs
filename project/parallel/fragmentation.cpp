...
// Используем reducer векторы, чтобы избегать гонок данных при параллелизме
...
void high_level_analysis::GetSolution()
{
	...
	for (int i = 0; i < (level + 1); ++i)
	{
		...
		cilk_for (int j = 0; j < current_boxes.size(); ++j) // используем cilk_for для рапараллеливания внутреннего цикла
